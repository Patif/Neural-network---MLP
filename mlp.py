...
class MLP:
    ...
    def __init__(self, structure, initialization=0):
        self.__w = []
        self.__bias = []
        self.__layer_activ_functions = []
        for i, (layer_size, activ_function) in enumerate(structure[0:-1]):
            if initialization == 0:  # He
                multiplier = np.sqrt(2 / layer_size)
            if initialization == 1:  # Xavier
                multiplier = np.sqrt(2 / (layer_size + structure[i + 1][0]))
            self.__w.append(np.random.randn(structure[i + 1][0], layer_size) * multiplier)
            self.__bias.append(np.random.randn(structure[i + 1][0], 1) * multiplier)
    ...
    def train(self, x_train, y_train, batch_size, alpha, x_val, y_val, epochs=20, gamma=0.7, optimizer=0, beta_1=0.9,
        ...
        while epoch < epochs:
            ...
            adadelta_d_w_last, adadelta_d_bias_last, adadelta_E_w_last, adadelta_E_bias_last = None, None, None, \
                                                                                               None, None, None, \
                                                                                               None, None, None, \
                                                                                               None, None, None
            t = 1
            for i in range(batches):
                ...
                delta_w_last, delta_bias_last, grads_w_last, grads_bias_last, m_w, v_w, m_bias, v_bias, \
                adadelta_d_w_last, adadelta_d_bias_last, adadelta_E_w_last, \
                adadelta_E_bias_last = self.learn(
                    ...
                    adadelta_E_w_last=adadelta_E_w_last, adadelta_E_bias_last=adadelta_E_bias_last)
                t += 1
            acc = self.assessment(x_val, y_val)
            #print("Celnosc(walidacyjny): {} Epoka: {}".format(acc, epoch + 1))
            if best_w is None or acc > highest_acc:
                best_w = deepcopy(self.__w)
                best_bias = deepcopy(self.__bias)
                highest_acc = acc
            epoch += 1
        self.__w = best_w
    ...
    def learn(self, x_train, y_train, batch_size, alpha, gamma, optimizer=0,
              ...
              adadelta_d_w_last=None, adadelta_d_bias_last=None, adadelta_E_w_last=None, adadelta_E_bias_last=None):
        delta_w = []
        delta_bias = []
        grads_w = []
        grads_bias = []
        ...
        for m in range(len(self.__bias)):
            ...
            adadelta_E_w.append(np.zeros(self.__w[m].shape))
            adadelta_E_bias.append(np.zeros(self.__bias[m].shape))
        ...
        for index, data in enumerate(x_train):
            ...
            while j >= 0:
                ...
                if z_matrices:
                    if optimizer == 2 and delta_w_last is not None:  # momentum nesterov'a
        ...
        for j in range(len(self.__bias)):
            if optimizer == 4:  # adadelta
                adadelta_E_w[j] = adadelta_gamma * (0 if adadelta_E_w_last is None else adadelta_E_w_last[j]) \
                                  + (1 - adadelta_gamma) * (delta_w[j] ** 2)
                adadelta_E_bias[j] = adadelta_gamma * (0 if adadelta_E_bias_last is None
                                                       else adadelta_E_bias_last[j]) + (1 - adadelta_gamma) * (
                                             delta_bias[j] ** 2)

                delta_w[j] *= np.sqrt((0 if adadelta_d_w_last is None else adadelta_d_w_last[j]) + epsilon) / np.sqrt(
                    adadelta_E_w[j] + epsilon)
                delta_bias[j] *= np.sqrt(
                    (0 if adadelta_d_bias_last is None else adadelta_d_bias_last[j]) + epsilon) / np.sqrt(
                    adadelta_E_bias[j] + epsilon)
                ...
                if optimizer in [0, 1, 2]:  # pomniejszamy wartosc wspolczynnika uczenia o rozmiar batch'a
        ...
        return delta_w, delta_bias, grads_w, grads_bias, m_w, v_w, m_bias, v_bias, adadelta_d_w, adadelta_d_bias, adadelta_E_w, adadelta_E_bias
